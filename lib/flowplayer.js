...
(function() {
  ...
  window.addEventListener('load', function() {
     ...
     var urlInput = document.querySelector('.load-from-url input');
     var urlInputBtn = document.querySelector('.load-from-url-btn');

     video.setAttribute('autoplay', 'true');

     urlInputBtn.onclick = () => {
        videoSource.setAttribute('src', urlInput.value);
        video.load();
      };

      var diskInput = document.querySelector('.load-from-disk input');
      
      diskInput.oninput = ()=>{
        var videoFile = diskInput.files[0];
        const videoSourceURL = URL.createObjectURL(videoFile);
    ...
    1: [function (_dereq_, module, exports) {
      ...
      (function (Buffer) {
        ...
        flowplayer(function (api, root) {
          ...
          var source = vidPlayer.querySelector('source');

          vidPlayer.addEventListener('loadeddata', ()=>{
            api.video.src = source.getAttribute('src');
            api.video.sources[0].src = source.getAttribute('src');
            api.video.duration = vidPlayer.duration;
            api.video.time = vidPlayer.currentTime;
            api.video.seekable = vidPlayer.duration;
          });
         ...
         <div class="fp-controls">\
          ...
          // refresh UI elements if video src is set externally
        var vidPlayer = document.querySelector("video");

        vidPlayer.onloadeddata = () => {
            document.querySelector(".fp-duration").innerHTML = format(vidPlayer.duration);
        };
        ...
        //         document.querySelector(".fp-duration").innerHTML = format(vidPlayer.duration);
        //         common.find(".fp-duration", root)[0].innerHTML = format(vidPlayer.duration);
        //         player.unbind("progress.dur");
        //     }
        //   });
        // });
        // observer.observe(vidPlayer, {
        ...
        // });
          ...
          api.on("ready", function (ev, api, video) {
            ...
            var duration = api.video.duration, // TODO: video URL is not changed here
